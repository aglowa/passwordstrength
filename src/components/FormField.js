...
// import React Component, Fragment and PropTypes
import React, { Component, Fragment } from 'react';
import PropTypes from 'prop-types';

class FormField extends Component {

    // initialize state, value empty, dirty checks that form has been toiched by user - default value false, errors empty table
    state = { value: '', dirty: false, errors: [] }
    // adding event handler to update the state value to the current input value on every change to the input
    hasChanged = e => {
        e.preventDefault();
        ...
        // check if isEmpty so value.length === 0
        const isEmpty = value.length === 0;
        // check requiredMissing if the state is dirty, field is required and isEmpty
        const requiredMissing = this.state.dirty && required && isEmpty;
        // declaring table for errors inside the function
        let errors = [];
